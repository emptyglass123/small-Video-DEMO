...
- (unsigned long)getFileSize:(NSString *)zipFile{
    unsigned long size;
    NSError * error = [[NSError alloc] init];
    NSDictionary * dic1 = [[NSFileManager defaultManager] attributesOfItemAtPath:zipFile error:&error];
    NSDictionary * dic2 = [[NSFileManager defaultManager] attributesOfFileSystemForPath:zipFile error:&error];
    NSLog(@"data.zip info: %@\n",dic1);
    NSLog(@"data.zip fileSystem info: %@\n",dic2);
    size = [[dic1 valueForKey:@"NSFileSize"] longValue];
    return size;
}

-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
    unsigned long size = [self getFileSize:zipFile];
	_unzFile = unzOpen( (const char*)[zipFile UTF8String],size);
	if( _unzFile )
	{
...
// 当写入成功是调用
- (void)OutputSucessMessage
{
    if (_delegate && [_delegate respondsToSelector:@selector(SendValueSuccess)]) {
        return [_delegate SendValueSuccess];
    }
}
